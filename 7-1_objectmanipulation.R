################################################################################
### R BASICS WORKSHOP                                                        ###
### PRESENTATION 7: MANIPULACIÓN DE OBJETOS - INDEXACIÓN                     ### 
###                                                                          ###
...
CO2 <- read.table(file.choose(), header=TRUE, row.names=1, sep="\t")
dim(CO2)
...
# La manera más comun de indexar matrices es por fila y por columna.
...
bat.data <- read.table(file.choose(), header=TRUE, sep="\t")
dim(bat.data)
class(bat.data) 
