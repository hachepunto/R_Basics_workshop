...
# Las figuras van a la carpeta "salidas", que se crea aquí si no existe:
...
# SOBRE EL FORMATO: la versión original de esta clase guardaba un TIFF de
# 10 x 10 pulgadas a 300 ppp. Un TIFF así son 3000 x 3000 pixeles SIN comprimir,
# es decir unos 36 MB por figura. Peor aún, el argumento 'compression' se ignora
# en el dispositivo por defecto de macOS ('quartz'), así que ahí no hay manera de
# hacerlo más chico. PNG comprime sin pérdida y da el mismo resultado en
# pantalla, así que es mejor opción mientras no te pidan TIFF explícitamente:
...
list.files("salidas/")
# Compara los dos archivos: son idénticos, pero el segundo se generó con 5
# líneas de código en vez de 100.
...
# *for* es la manera más común de construir bucles. Este tipo de bucle repite 
# un pedazo de código un número pre-determinado de veces.
...
# uno por uno. Solo tiene sentido en una sesión interactiva: cuando el archivo
# se corre por lotes nada más lo hace lento, así que aquí lo condicionamos con
...
for(i in 1:max.time)
{
	...
	if(trait.shift == 0) # Poco probable, pero si no lo cubrimos, en ese caso
	    COL <- "grey50"  # 'COL' conservaría el valor de la vuelta anterior
...
# OJO con el tamaño: cada valor numérico ocupa 8 bytes, así que esta matriz de
# un millón de valores pesa unos 8 MB. Si pusieras 50 millones de valores serían
# 400 MB, y la "Opción 1" de abajo tardaría horas. Cuando pruebes cosas de
# eficiencia, empieza siempre con un tamaño pequeño y ve subiendo.
...
# ¿Cómo calcular la abundancia total de individuos por sitio (fila)?


## Opción 1 - un bucle muy ineficiente ##
# El problema es 'c(abund.1, ...)': en cada vuelta R copia TODO el vector para
# crear uno nuevo un poco más largo. El costo crece con el cuadrado del número
# de vueltas.
...
# Aquí el vector se crea UNA vez con su tamaño final y el bucle solo rellena
...
## Opción 3 - vectorización ##
# Preguntamos por la suma de cada FILA (cada sitio), así que la función es
# 'rowSums', no 'colSums'. 'colSums' sumaría por especie, que es otra pregunta.
...
# Y para comparar, la abundancia total por ESPECIE (columna) sería:
spp.total <- colSums(M)
length(spp.total)


# La familia de funciones *apply* generaliza esta idea a cualquier función,
